...
import os
import tabular_mdp
from evaluation_utils import get_PR, oracle_value, evaluations_vtrace, evaluations_importancesampling, evaluations_firstorder, evaluations_MAML_mg_gae, policy_evaluation, evaluations_firstorder_mg_gae
import torch
...
def generate_trajectories(mdp, mu, num_simulations, T, intrinsic_reward=None):
    """
    ...
    for i in range(num_simulations):
        ...
        for t in range(T):
            ...
            rsum.append(r)
            if not intrinsic_reward is None:
                irsum.append(intrinsic_reward[s, a])           
            actions.append(a)
            states.append(s)
            s = s_next
        states.append(s_next)
        trajs.append({'states': states, 'actions': actions, 'rewards': rsum, 'ir':irsum})
    return trajs
...
def get_estimator(estimator):
    if estimator == 'exact':
        ...
        return evaluations_firstorder_mg_gae
    elif estimator == 'loaded':
        return evaluations_importancesampling
    elif estimator == 'dice':
        return evaluations_vtrace
    elif estimator == 'maml':
        return evaluations_MAML_mg_gae
    else:
        return None

def get_value(est, estimator, params_fast, P, R, mdp, T, num_traj, gamma, gae_lambda=0.9, detach_value_gradient=False, ir=None, noise=1.0):
    if est == 'exact':
        if ir is None:
            value_all = oracle_value(params_fast, P, R, gamma)
        else:
            value_all = oracle_value(params_fast, P, R+ir, gamma)
    else:
        mu = F.softmax(params_fast, dim=-1).detach().numpy()
        trajs_all = [generate_trajectories(mdp, mu, 1, T, ir) for _ in range(num_traj)]
        ...
        if ir is None:
            ...
            V_exact = policy_evaluation(P, R+ir, gamma, F.softmax(params_fast, dim=-1).detach())[0]
            size = V_exact.detach().numpy().size
            noise_level = noise
            V_exact = V_exact + torch.randn(V_exact.shape) * noise_level * torch.norm(V_exact).detach() / size
            if detach_value_gradient:
                V_bootstrapped = V_exact.clone().detach()#.clone()
            else:
                V_bootstrapped = V_exact.clone()
        ...
        for i in range(num_traj):
            trajs = trajs_all[i]
            value = estimator(params_fast, F.softmax(params_fast, dim=-1).detach(), T, gamma, V_bootstrapped, trajs, rho, c, gae_lambda=gae_lambda)
...
def main():
    ...
    detach_value_gradient = args.detach_value_gradient
    ...
    for i in range(independent_trials):
        ...
        intrinsic_reward = torch.zeros([ns, na], requires_grad=True)
        ...
        for step in range(step_num):     
            value = oracle_value(params_fast, P, R+intrinsic_reward, gamma)
        ...
        oracle_meta_gradient = torch.autograd.grad(value, intrinsic_reward)[0]
        ...
        for _ in range(same_trials):
            ...
            for step in range(step_num):
                # inner
                value = get_value(est=inner_est, estimator=inner_estimator, params_fast=params_fast, P=P, R=R, mdp=mdp, T=T, num_traj=inner_traj, gamma=gamma, ir=intrinsic_reward)
                
                prarams_inner = params_fast + lr * torch.autograd.grad(value, params_fast)[0]
                
                # hessian
                value = get_value(est=hessian_est, estimator=hessian_estimator, params_fast=params_fast, P=P, R=R, mdp=mdp, T=T, num_traj=hessian_traj, gamma=gamma, gae_lambda=gae_lambda, detach_value_gradient=detach_value_gradient, ir=intrinsic_reward, noise=noise)
            ...
            # outer
            value = get_value(est=outer_est, estimator=outer_estimator, params_fast=params_fast, P=P, R=R, mdp=mdp, T=T, num_traj=outer_traj, gamma=gamma)
            meta_gradient = torch.autograd.grad(value, intrinsic_reward)[0]
            meta_gradient_list.append(meta_gradient.reshape(-1))
            grad_corr.append(corr(meta_gradient.detach().numpy(), oracle_meta_gradient.detach().numpy()))

        meta_gradient = torch.stack(meta_gradient_list, dim=0)
        grad_corr = sum(grad_corr)/len(grad_corr)
        grad_var = get_variance(meta_gradient)
        corr_list.append(grad_corr)
        var_list.append(grad_var.item())
    corr_list = np.array(corr_list)
    var = np.array(var_list)
    np.save(log_dir+'/corr', corr_list)
    np.save(log_dir+'/var', var)
